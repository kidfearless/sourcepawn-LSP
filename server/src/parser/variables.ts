...
export function FindVariables(textDocument: TextDocument, customVariableList:string[]): void
{
	...
	FindCustomVariables(textDocument, text, customVariableList);
}
...
function FindCustomVariables(textDocument: TextDocument, text: string, customVariableList:string[]): void
{
	for(let i = 0; i < customVariableList.length; ++i)
	{
		// shorthand copy of the current datatype
		let dataType:string = customVariableList[i];
		// Creates a regular expression to find any declaration statements including ones where there might be multiple variables declared.
		let dataTypePattern = new RegExp(`${dataType}\\s+[\\w\\s,=+\\-*/\\d.()\\[\\]]+;`, 'g');
		// (float|bool|char|int)\s+[\w\s,=+\-*/\d.()\[\]]+;
		let variableNamePattern = /\w+/;
